...
如果使用自定义 Anthropic-compatible endpoint：

```bash
export ANTHROPIC_BASE_URL="https://api.example.com/anthropic"
```

未设置 `ANTHROPIC_AUTH_TOKEN` 时，也支持 `ANTHROPIC_API_KEY`。
...
- `agent_kernel/model_provider.py`：fake provider 与 Anthropic-compatible provider。
